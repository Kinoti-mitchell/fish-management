-- Fix Duplicate Key with ON CONFLICT - Handle Duplicates Gracefully
-- This uses PostgreSQL's ON CONFLICT to handle duplicate key violations
...
-- Create approve_transfer function with ON CONFLICT handling
...
BEGIN
    ...
    -- Add inventory to destination storage using ON CONFLICT
    ...
    DO UPDATE SET
        total_pieces = sorting_results.total_pieces + EXCLUDED.total_pieces,
        total_weight_grams = sorting_results.total_weight_grams + EXCLUDED.total_weight_grams,
        updated_at = NOW();
...
-- Success message
SELECT 'Duplicate key fix with ON CONFLICT applied - should handle duplicates gracefully!' as status;
