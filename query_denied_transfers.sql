-- Query to find and work with the denied transfers for User f5946671
-- Run this in your Supabase SQL editor

-- First, let's find these specific denied transfers
SELECT 
    t.id,
    t.from_storage_location_id,
    t.to_storage_location_id,
    t.size_class,
    t.quantity_kg,
    t.quantity_pieces,
    t.status,
    t.notes,
    t.created_at,
    t.approved_by,
    t.approved_at,
    sl_from.name as from_storage_name,
    sl_to.name as to_storage_name
FROM transfers t
LEFT JOIN storage_locations sl_from ON t.from_storage_location_id = sl_from.id
LEFT JOIN storage_locations sl_to ON t.to_storage_location_id = sl_to.id
WHERE t.notes LIKE '%Transfer from Cold Storage B - Sizes: 1, 2, 4%'
AND t.created_at::date = '2025-09-18'
ORDER BY t.size_class, t.created_at;

-- If you want to reset these transfers back to pending status:
-- UPDATE transfers 
-- SET 
--     status = 'pending',
--     approved_by = NULL,
--     approved_at = NULL,
--     updated_at = NOW()
-- WHERE notes LIKE '%Transfer from Cold Storage B - Sizes: 1, 2, 4%'
-- AND created_at::date = '2025-09-18'
-- AND status = 'denied';

-- If you want to approve these transfers instead:
-- UPDATE transfers 
-- SET 
--     status = 'approved',
--     approved_by = 'f5946671'::uuid,  -- Replace with actual user ID
--     approved_at = NOW(),
--     updated_at = NOW()
-- WHERE notes LIKE '%Transfer from Cold Storage B - Sizes: 1, 2, 4%'
-- AND created_at::date = '2025-09-18'
-- AND status = 'denied';

-- If you want to delete these transfers completely:
-- DELETE FROM transfers 
-- WHERE notes LIKE '%Transfer from Cold Storage B - Sizes: 1, 2, 4%'
-- AND created_at::date = '2025-09-18'
-- AND status = 'denied';
