-- Fix Transfer Function to Merge Fish by Size Class
-- This transfers fish by adding to existing sizes in destination location

-- Step 1: Drop the existing transfer function
DROP FUNCTION IF EXISTS transfer_inventory_between_storage CASCADE;

-- Step 2: Create a new transfer function that merges by size class
...
DECLARE
    ...
    v_transferred_weight_grams INTEGER;
...
BEGIN
    ...
    FOR v_batch_record IN 
        SELECT 
            ...
            (array_agg(sr.sorting_batch_id))[1] as sorting_batch_id
        FROM sorting_results sr
        JOIN sorting_batches sb ON sr.sorting_batch_id = sb.id
        WHERE sb.storage_location_id = p_from_storage_location_id 
        AND sb.status = 'completed'
    ...
    LOOP
        ...
        INSERT INTO sorting_results (
            ...
            v_batch_record.size_class,
            v_batch_record.total_pieces,
            v_batch_record.total_weight_grams,
            v_batch_record.sorting_batch_id,
            NOW(),
            NOW()
        );
