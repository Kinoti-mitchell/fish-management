-- Ensure Inventory Storage Accuracy with FIFO Order Management
-- This script ensures inventory shows all storage locations with accurate capacity
-- and implements FIFO (First In, First Out) for order processing

-- Step 1: Update storage capacity from actual inventory
SELECT 'Updating storage capacity from actual inventory...' as status;
SELECT update_storage_capacity_from_inventory();

-- Step 2: Ensure all storage locations are properly set up
...
-- Step 3: Create function to get inventory with FIFO batch ordering
...
-- Step 4: Create function to process FIFO order fulfillment
...
-- Step 5: Create function to reduce inventory when order is approved
...
-- Step 8: Show current storage capacity status
...
-- Step 9: Final status
SELECT 
    'Inventory storage accuracy system completed!' as status,
