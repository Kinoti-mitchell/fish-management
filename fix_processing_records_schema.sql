-- Fix Processing Records Schema to Allow NULL processing_yield
-- This is a more direct approach - modify the schema constraint

-- First, let's check the current constraint and modify it
-- We'll make processing_yield nullable to avoid the constraint issue

-- Drop the NOT NULL constraint on processing_yield
ALTER TABLE processing_records ALTER COLUMN processing_yield DROP NOT NULL;

-- Set a default value for processing_yield
...
-- Now recreate the functions with the corrected approach
...
BEGIN
    ...
    ELSE
        ...
        -- Create a dummy processing record (now processing_yield can be NULL)
        INSERT INTO processing_records (
            ...
            NOW()::date,
            p_approved_by,
            0.00,  -- pre_processing_weight
            0.00,  -- post_processing_weight
            0.00,  -- processing_waste
            NULL,  -- processing_yield (now allowed to be NULL)
            0,     -- ready_for_dispatch_count
            NOW(),
            NOW()
...
-- Success message
SELECT 'Processing records schema fixed - processing_yield constraint removed!' as status;
